...
object UserActionStreamingEtl {
  ...
  def main(args: Array[String]): Unit = {
    val Array(host, port) = Array(s"${args(0)}", s"${args(1)}")

    val batchInterval = Milliseconds(10000)

    // Create the context and set the batch size
    val sparkConf = new SparkConf().setAppName("UserActionStreamingEtl").setMaster("local[4]")
    val outputPath = sparkConf.get("spark.user.action.outputPath",
      "hdfs://starl:9000/user/hadoop-twq/dw-course/streaming-etl/user-action-parquet")
    ...
    val userActionStream = flumeEventStream.map(sparkFlumeEvent => {
      ...
      val hostname = headers.get("host").toString
    ...
    userActionStream.foreachRDD(userActionRdd => {
      val spark = SparkSession.builder.config(userActionRdd.sparkContext.getConf).getOrCreate()
